...
DIRECTORY = ROOT / "examples/cloud_final"
...
def verify(directory=DIRECTORY):
    records = {name: load(name, directory) for name in (
        "order", "refund", "rag", "memory_a", "memory_b", "discount",
        "browser", "gateway_failure",
    )}
    guard = load("rag_guard", directory) if (directory / "rag_guard.json").exists() else records["rag"]
    source = load("source_verification", directory)
    digest = hashlib.sha256((ROOT / "main.py").read_bytes()).hexdigest()
    same_source = source["uploaded_main_sha256"] == digest and all(
        record["source_sha256"] == digest for record in [*records.values(), guard])
    same_runtime = len({r["runtime_arn"] for r in [*records.values(), guard]}) == 1
    http_ok = all(r.get("http_status") == 200 for r in [*records.values(), guard])
    ...
    scenarios = {
        ...
        "knowledge_retrieval": bool(rag_results) and all(successful(r) for r in rag_results)
            and "not configured" not in rag_text
            and all(word in rag_text for word in ("same-day", "15%", "priority"))
            and all(word in records["rag"]["output"].get("response", "").lower()
                    for word in ("same-day", "15%", "priority")),
    ...
    logs = load("runtime_logs", directory)
    guard_text = json.dumps([r for n, r in tool_results(guard) if n == "search_knowledge_base"]).lower()
    corrections = {
        "missing_kb_guard": "kb_id is empty or missing" in guard_text
            and "configure KB_ID" in guard["output"]["response"],
    ...
    if directory.name == "cloud_final":
        cli_ok = cli_ok and cli.get("runtime_arn") == records["order"]["runtime_arn"]
    report = {
        "checked_utc": datetime.now(timezone.utc).isoformat(),
        "scope": "Saved AWS Runtime invocation responses; verification does not invoke AWS",
        ...
        "missing_kb_guard_version": guard["runtime_version"],
        ...
        "rag_blocker": load("rag_permission", directory),
...
if __name__ == "__main__":
    ...
    parser.add_argument("--directory", type=Path, default=DIRECTORY,
                        help="Directory containing one complete recorded deployment run")
